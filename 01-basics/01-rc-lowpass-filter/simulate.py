"""
RC Low-Pass Filter: LTspice batch simulation + Bode plot + verification.
...
NET_FILE    = SCRIPT_DIR / "rc_lowpass.net"
RAW_FILE    = SCRIPT_DIR / "rc_lowpass.raw"
...
if not RAW_FILE.exists():
    print(f"ERROR: {RAW_FILE} not created. Check LTspice output above.")
    sys.exit(1)
...
# Frequency axis — for AC simulations 'frequency' is a named trace
freq_raw = raw.get_wave("frequency")
freq = np.abs(freq_raw)        # complex in raw; magnitude = actual frequency

# Output voltage (AC phasor)
vout = raw.get_wave("V(vout)")
mag_db = 20 * np.log10(np.abs(vout))

# ── find -3 dB point ──────────────────────────────────────────────────────────
ref_db = mag_db[0]             # magnitude at lowest frequency (should be ~0 dB)
target_db = ref_db - 3.0
idx = np.argmin(np.abs(mag_db - target_db))
f_sim = freq[idx]
print(f"Simulated -3 dB cutoff: {f_sim:.1f} Hz  (ref={ref_db:.2f} dB, at idx={idx})")

# ── Bode plot ─────────────────────────────────────────────────────────────────
fig, ax = plt.subplots(figsize=(8, 5))
ax.semilogx(freq, mag_db, "b-", linewidth=2, label="Simulated")
ax.axhline(ref_db - 3, color="r", linestyle="--", alpha=0.8, label="-3 dB line")
...
ax.set_title("RC Low-Pass Filter — Bode Magnitude")
ax.legend()
