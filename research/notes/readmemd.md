...
deprecated: false
summary: 'README for the ''learn-fpga'' open-source teaching repository (content matches
  BrunoLevy/learn-fpga; the assigned URL filipkosecek/learn-fpga appears to be a fork
  retaining the same README) providing free FPGA/RISC-V teaching material costing
  about 0/student in hardware. Centerpiece is FemtoRV, a minimalistic RISC-V CPU family
  with easy-to-read Verilog written directly from the RISC-V spec: the smallest variant
  (quark, RV32I) is ~400 documented lines of Verilog (~100 lines uncommented), scaling
  up to petitbateau (RV32IMFC) with a companion SoC (UART, LED matrix, OLED, SPI RAM,
  SDCard drivers) that fits in under 1280 LUTs on a Lattice IceStick. Includes a step-by-step
  tutorial series (''From Blinky to RISC-V'') teaching students to build a RISC-V
  core from a blinking LED up to a functional processor with graphics, then a pipelined
  version. Also links a curated list of other open-source RISC-V soft-core and FPGA-tooling
  projects (picorv32, VexRiscv/NaxRiscv, SERV, DarkRiscV, kianRiscV, LiteX). Directly
  corroborates the UTAR RISC-V dissertation''s framing of RISC-V as an accessible,
  low-cost platform for student processor-design projects, and gives ready-made, low-cost,
  documented reference implementations (FemtoRV/quark) as an alternative or supplement
  to building a RISC-V core from scratch.'
---
