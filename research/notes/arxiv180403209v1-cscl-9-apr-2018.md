---
title: arXiv:1804.03209v1  [cs.CL]  9 Apr 2018
id: arxiv180403209v1-cscl-9-apr-2018
tags:
- btech-ece-projects-259aee
- tinyml
- dataset
created: '2026-09-25T03:17:36.370020Z'
updated: '2026-09-25T03:18:10.418485Z'
source: https://arxiv.org/abs/1804.03209
source_domain: arxiv.org
fetched_at: '2026-09-25T03:17:36.369226Z'
...
content_type: paper
deprecated: false
summary: Pete Warden (Google Brain, 2018) primary-source paper introducing the Google
  Speech Commands Dataset, the standard benchmark used to train/evaluate keyword-spotting
  (limited-vocabulary speech recognition) models — the exact dataset used in the ESP32/TinyML
  speech-recognition study. Motivates why keyword spotting needs a dedicated, openly
  licensed (CC BY 4.0) dataset distinct from full-sentence ASR corpora, to enable
  reproducible, apples-to-apples comparison across small on-device models. Describes
  data collection/verification methodology and reports baseline model accuracy results.
  Foundational citation for any B.Tech ECE project involving voice/keyword-command
  recognition on embedded hardware.
raw_file: raw/arxiv180403209v1-cscl-9-apr-2018.pdf
doi: arXiv:1804.03209
---

*Suggested by [[deploying-real-time-speech-recognition-on-esp32-using-tinyml-and-edge-impulse-sp]] — cited as source dataset (Google Speech Commands) for TinyML speech recognition*
...
systems.
Discusses why this task is an interesting
...
ble accuracy metrics for this task.
Describes how
the data was collected and veriﬁed, what it contains,
previous versions[2] and properties.
Concludes by
...
ties or corporations to pursue.
People working in
...
public domain).
This licensing makes it very easy
to build on top of.
...
ferent.
There are some promising datasets to sup-
...
facturers.
By using a publicly-available task that
...
https://aiyprojects.withgoogle.com/֒
→open_speech_recording
...
find ${BASEDIR}/oggs -iname "*.ogg" -size֒
→-5k -delete
...
find ${BASEDIR}/oggs -iname "*.ogg" -print0֒
→
| xargs -0 basename -s .ogg | xargs֒
→
-I {} ffmpeg -i ${BASEDIR}/oggs/{}.֒
→ogg -ar 16000 ${BASEDIR}/wavs/{}.wav
...
the overall volume of the utterance.
From experi-
...
git clone https://github.com/petewarden/֒
→extract_loudest_section tmp/֒
→extract_loudest_section
cd tmp/extract_loudest_section
make
cd ../..
mkdir -p ${BASEDIR}/trimmed_wavs
/tmp/extract_loudest_section/gen/bin/֒
→extract_loudest_section ${BASEDIR}’/֒
...
scipy.io.wavfile.write(’/tmp/white_noise.֒
→wav’, 16000, np.array(((acoustics.֒
→generator.noise(16000*60, color=’֒
→white’))/3) * 32767).astype(np.int16֒
→))
scipy.io.wavfile.write(’/tmp/pink_noise.wav֒
→’, 16000, np.array(((acoustics.֒
→generator.noise(16000*60, color=’֒
→pink’))/3) * 32767).astype(np.int16)֒
→)
To
distinguish
these
...
a
specially-named
"_background_noise_" folder, in the root of the
archive.
6
Properties
...
ﬁtting.
The dataset download includes a text ﬁle
...
changes.
The testing_list.txt ﬁle contains the
...
audio and score them on multiple metrics.
Here’s
what the baseline model trained with V2 data pro-
duces:
49.0% matched, 46.0% correctly, 3.0%֒
→wrongly, 0.0% false positives
To produce this result, I ran the following bash
script against the 10 minute streaming test audio clip
and ground truth labels:
bazel run tensorflow/examples/֒
→speech_commands:freeze -- --֒
→start_checkpoint=/tmp/֒
→speech_commands_train/conv.ckpt֒
→-18000 --output_file=/tmp/֒
→v2_frozen_graph.pb
bazel run tensorflow/examples/֒
→speech_commands:֒
→test_streaming_accuracy -- --graph=/֒
→tmp/v2_frozen_graph.pb --wav=/tmp/֒
→speech_commands_train/streaming_test֒
→.wav --labels=/tmp/֒
→speech_commands_train/conv_labels.֒
→txt --ground_truth=/tmp/֒
→speech_commands_train/֒
→streaming_test_labels.txt
...
tics.
One of the other conﬁgurable features of the
...
speakers.
Training the default convolution model
...
improvement in accuracy over V1.
The full set of
...
python tensorflow/examples/speech_commands/֒
→train.py --data_url=\protect\vrule֒
→width0pt\protect\href{http://֒
→download.tensorflow.org/data/֒
→speech_commands_v0.01.tar.gz}{http֒
→://download.tensorflow.org/data/֒
...
→width0pt\protect\href{http://֒
→download.tensorflow.org/data/֒
→speech_commands_v0.02.tar.gz}{http֒
→://download.tensorflow.org/data/֒
→speech_commands_v0.02.tar.gz}
The results of these commands are available as pre-
trained checkpoints[20].
...
→width0pt\protect\href{http://֒
→download.tensorflow.org/data/֒
→speech_commands_v0.01}{http://֒
→download.tensorflow.org/data/֒
→speech_commands_v0.0{1},1}.tar.gz --֒
→start_checkpoint=${HOME}/֒
→speech_commands_checkpoints/conv-v֒
→{1,2}.ckpt-18000
7.4
Applications
...
music models.
Keyword Spotting Through Image
...
[10] (2017) Hey siri:
An on-device dnn-powered voice trigger for apple’s personal assistant. [Online].
...
[Online].
Available:
https://github.com/tensorﬂow/tensorﬂow/blob/master/tensorﬂow/examples/speech_commands/test_streaming_accu
[17] (2018)
...
[Online].
Available:
https://github.com/tensorﬂow/tensorﬂow/blob/master/tensorﬂow/examples/speech_commands/recognize_commands
10
...
Proceedings of the 22Nd ACM International Conference on Multimedia, ser. MM ’14.
New York, NY,
